...
enum ToeError { NO_ERROR, OUT_OF_BOUNDS, ALREADY_FILLED };

/*
 * Represents an state of an point and
 * victory condition
 */
enum ToeState { STATE_EMPTY, STATE_TIC, STATE_TAC, STATE_DRAW };
 ...
 * is undefined.
 */
enum ToeError toe_move(struct Toe *g, char v, char h);

enum ToeState toe_victory(struct Toe const *const t);

void toe_display(struct Toe const *const t);

inline void toe_clear_screen() { fputs("\033[2J\033[H", stdout); }

inline void toe_presentation(struct Toe const *const t) {
  toe_clear_screen();
  toe_display(t);
}
