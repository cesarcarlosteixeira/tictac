...
static inline bool bounded_(int x) {
	return x >= 0 && x <= 2;
}

static inline enum ToeState check_triple_(
	enum ToeState const tbl[const static 3]
) {
...
static inline bool all_filled_(
	enum ToeState const tbl[const static 3][3]
) {
...
static inline char state_repr_(
	enum ToeState ts
) {
	switch (ts) {
	case STATE_EMPTY:
...
void toe_display(
	...
	for (size_t i = 0; i < 3; i++) {
		printf(
			values, 
			letters[i],
			state_repr_(t->table[0][i]),
			state_repr_(t->table[1][i]),
			state_repr_(t->table[2][i])
		);
	}
}
